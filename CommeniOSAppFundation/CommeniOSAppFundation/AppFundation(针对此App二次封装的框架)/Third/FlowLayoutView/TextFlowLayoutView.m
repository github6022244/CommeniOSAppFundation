...
- (void)setupUIWithFrame:(CGRect)frame {
    ...
    for (int i = 0; i < total; i++) {
        ...
        if ([keyArr containsObject:@"itemSelectColor"]) {
            ...
            [selectBtn setBackgroundImage:[UIImage imageWithColor:selectColor] forState:UIControlStateHighlighted];
        }
        if ([keyArr containsObject:@"titleSelectColor"]) {
            ...
            [selectBtn setTitleColor:selectTitleColor forState:UIControlStateHighlighted];
        }
        /// 为按钮增加Action
...
- (void)tranformWithSender:(UIGestureRecognizer *)sender {
    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        //QMUILog(nil, @"center = %@",NSStringFromCGPoint(btn.center));
