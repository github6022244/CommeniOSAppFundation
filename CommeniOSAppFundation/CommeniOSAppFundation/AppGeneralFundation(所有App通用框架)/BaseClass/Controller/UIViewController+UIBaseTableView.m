...
//    }
    ...
    if (pageModel.loadAndLessDataCount) {
        ...
        [self tableViewFooterEndRefreshingWithResetNoMoreData];
    }
}

// 根据传入的网络model 控制 .mj_footer 的文字状态
- (void)tableViewFooterConfigNoMoreDataWithReslutModel:(GGBaseRequestReslutModel *)resModel {
    if (resModel.isLoadToLastPage) {
        // 上拉加载 并且没有更多数据
        [self tableViewFooterEndRefreshingWithNoMoreData];
    } else {
        [self tableViewFooterEndRefreshingWithResetNoMoreData];
...
- (void)tableViewFooterEndRefreshingWithResetNoMoreData {
...
- (void)pv_addTableView {
    ...
    // 解决 UITableViewStyleGrouped 顶部偏移问题
    [self.tableView gg_fixGroupedTableViewExcursion];
